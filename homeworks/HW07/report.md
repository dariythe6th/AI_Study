...
\- Разбиение: train/test (80/20, `random\_state=42`, `stratify=y` для сохранения пропорций классов)
...
&nbsp;  - Стратегия: `most\_frequent` (предсказывает самый частый класс)
...
&nbsp;  - Подбираемые параметры: `max\_depth` \[3, 5, 10], `min\_samples\_leaf` \[1, 5, 10]
...
&nbsp;  - Подбираемые параметры: `n\_estimators` \[50, 100], `max\_depth` \[5, 10], `max\_features` \['sqrt', 'log2']
...
&nbsp;  - Подбираемые параметры: `max\_iter` \[50, 100], `learning\_rate` \[0.1, 0.3], `max\_depth` \[3, 5], `l2\_regularization` \[0, 0.1]

&nbsp;  - Дополнительные настройки: `max\_bins=128`, `early\_stopping=True`, `n\_iter\_no\_change=5`
...
\*\*Устойчивость:\*\* Для проверки устойчивости был проведен дополнительный эксперимент с RandomForest (как второй лучшей модели) при разных `random\_state` \[42, 123, 456, 789, 999]. Результаты показали стабильность:
...
2\. \*\*Контроль сложности критически важен:\*\* Для DecisionTree ограничение глубины (`max\_depth=5`) и минимального количества образцов в листьях (`min\_samples\_leaf=10`) позволило избежать переобучения и получить разумный баланс bias-variance.



3\. \*\*Честный ML-протокол требует дисциплины:\*\* Фиксированный train/test split с `random\_state`, стратификация, подбор гиперпараметров только на train через CV, однократная оценка на test - все эти шаги обеспечивают надежную и воспроизводимую оценку моделей.
...
5\. \*\*Бустинг может быть оптимальным выбором:\*\* HistGradientBoosting показал наилучшие результаты, демонстрируя преимущество последовательного улучшения модели. При этом важно использовать регуляризацию (`l2\_regularization`) и раннюю остановку для предотвращения переобучения.
