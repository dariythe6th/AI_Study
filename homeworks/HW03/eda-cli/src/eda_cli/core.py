...
def summarize_dataset(
    ...
    for name in df.columns:
        ...
        # Считаем категориальными/объектными
        is_categorical = bool(ptypes.is_object_dtype(s) or isinstance(s.dtype, pd.CategoricalDtype))
        ...
        columns.append(
            ColumnSummary(
                ...
                is_categorical=is_categorical,  # <-- ДОБАВЛЕНО
                min=min_val,
                max=max_val,
                mean=mean_val,
                std=std_val,
...
def compute_quality_flags(
        ...
        cardinality_threshold: int = 50,  # <-- НОВЫЙ ПАРАМЕТР
) -> Dict[str, Any]:
    """
    ...
    # 2. НОВАЯ ЭВРИСТИКА 1: Константные колонки
    has_constant = any(col.unique <= 1 for col in summary.columns)
    flags["has_constant_columns"] = has_constant

    # 3. НОВАЯ ЭВРИСТИКА 2: Высокая кардинальность категорий
    has_high_cardinality = False
    for col in summary.columns:
        # Проверяем только те, которые считаем категориальными/объектными
        if col.is_categorical and col.unique > cardinality_threshold:
            has_high_cardinality = True
            break

    flags["has_high_cardinality"] = has_high_cardinality
    flags["cardinality_threshold"] = cardinality_threshold  # Сохраняем порог
    ...
    # Учитываем новые факторы (за каждый вычитаем 0.1)
    if has_constant:
        score -= 0.1
        ...
        rows.append(
            {
                ...
                "is_categorical": col.is_categorical,  # <-- ДОБАВЛЕНО
                "min": col.min,
                "max": col.max,
                "mean": col.mean,
                "std": col.std,
